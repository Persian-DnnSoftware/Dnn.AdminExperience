    ...
    function ($, ko, moment, ut, sf, cf, extension, persistent, eventEmitter, iconLoader, Gateway) {
        ...
        var util = {
            ...
            closePersonaBar: function handleClosePersonarBar(callback, keepSelection) {
                ...
                $activePanel.animate({ right: -860 }, 189, 'linear', function () {//dnnsoftware.ir
                    $('.socialpanel').css({ right: -860 }).hide();//dnnsoftware.ir
                    $mask.animate({
                        opacity: 0.0
                    }, 200, function () {
            ...
            loadPanel: function handleLoadPanel(identifier, params) {
                ...
                if ($mask.css("display") === 'none') {
                    ...
                    $mask.animate({
                        ...
                        $panel.show().delay(100).animate({ right: personaBarMenuWidth }, 189, 'linear', function () {//dnnsoftware.ir
                            inAnimation = false;
                    ...
                    if (activePath !== path) {
                        ...
                        $activePanel.fadeOut("fast", function handleHideCurrentPanel() {
                            $panel.css({ right: personaBarMenuWidth }).fadeIn("fast", function handleShowSelectedPanel() {//dnnsoftware.ir
        ...
        util.asyncParallel([
                ...
                function showPersonaBar(callback) {
                    ...
                    if ($parentBody.hasClass('dnnEditState')) {
                        $personaBar.css({ right: 0, 'display': 'block' });//dnnsoftware.ir
                        $parentBody.animate({ marginRight: personaBarMenuWidth }, 1, 'linear', onShownPersonaBar);//dnnsoftware.ir
                        callback();
                    } else {
                        $iframe.width(personaBarMenuWidth);
                        $personaBar.show();
                        ...
                        $personaBar.css({ right: 0, 'display': 'block' });//dnnsoftware.ir
                        $parentBody.animate({ marginRight: personaBarMenuWidth }, 1, 'linear', onShownPersonaBar);//dnnsoftware.ir
                        $personaBar.animate({ right: 0 }, 1, 'linear', callback);//dnnsoftware.ir
                    }
